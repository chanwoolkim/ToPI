...
keep if row_1 != .
save "`p'-homo_subpop-agg-1", replace

svmat `p'R_3, names(col)
rename `p'R_3num row_3
keep row_3 `p'R_3coeff `p'R_3pval
keep if row_3 != .
save "`p'-homo_subpop-agg-3", replace
}

cd "$homo_subpop_working"

use ehscenter-homo_subpop-agg-1, clear

foreach p of global programs {
	merge 1:1 row_1 using `p'-homo_subpop-agg-1, nogen nolabel
}

rename row_1 row
save agg-homo_subpop-1, replace

use ehscenter-homo_subpop-agg-3, clear

foreach p of global programs {
	merge 1:1 row_3 using `p'-homo_subpop-agg-3, nogen nolabel
}

rename row_3 row
save agg-homo_subpop-3, replace
