...
foreach t of global ehs_type {
	...
	merge 1:1 id using ehs`t'-merge, nogen nolabel keep(match)
...
merge 1:1 id using ihdp-merge, nogen nolabel keep(match)
...
merge 1:1 id using abc-merge, nogen nolabel keep(match)
...
merge 1:1 id using care`t'-merge, nogen nolabel keep(match)
...
cd "$data_analysis"
save care`t'-homo-merge, replace
}
