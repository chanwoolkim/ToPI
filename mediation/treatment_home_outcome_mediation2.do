...
local interaction_colnames theta theta_dup R R_dup theta_R theta_R_dup
local interaction_rownames ""EHS(Center) - PPVT" "EHS(Home) - PPVT" "EHS(Mixed) - PPVT" "EHS(All) - PPVT" "IHDP - PPVT" "IHDP - SB" "ABC - SB" "CARE (Both) - SB" "CARE (Home) - SB" "CARE (All) - SB""

local mediation_table_colnames theta theta_dup R R_dup
local mediation_table_rownames  ""EHS(Center) - PPVT" "EHS(Home) - PPVT" "EHS(Mixed) - PPVT" "EHS(All) - PPVT" "IHDP - PPVT" "IHDP - SB" "ABC - SB" "CARE (Both) - SB" "CARE (Home) - SB" "CARE (All) - SB""

qui matrix interaction_coef_se = J(10, 6, .)
qui matrix colnames interaction_coef_se = `interaction_colnames'
qui matrix rownames interaction_coef_se = `interaction_rownames'
qui matrix interaction_p = J(10, 6, .)
qui matrix colnames interaction_p = `interaction_colnames'
qui matrix rownames interaction_p = `interaction_rownames'

qui matrix mediation_table_coef_se = J(10, 4, .)
qui matrix colnames mediation_table_coef_se = `mediation_table_colnames'
qui matrix rownames mediation_table_coef_se = `mediation_table_rownames'
qui matrix mediation_table_p = J(10, 4, .)
qui matrix colnames mediation_table_p = `mediation_table_colnames'
qui matrix rownames mediation_table_p = `mediation_table_rownames'
...
foreach t of global ehs_type {
	...
	quietly {
		matrix list r(table)
		matrix r = r(table)
		matrix interaction_coef_se[`row',1] = r[1,1]
		matrix interaction_coef_se[`row',2] = r[2,1]
		matrix interaction_coef_se[`row',3] = r[1,3]
		matrix interaction_coef_se[`row',4] = r[2,3]
		matrix interaction_coef_se[`row',5] = r[1,2]
		matrix interaction_coef_se[`row',6] = r[2,2]
		matrix interaction_p[`row',1] = r[4,1]
		matrix interaction_p[`row',3] = r[4,3]
		matrix interaction_p[`row',5] = r[4,2]

		matrix mediation[`row',1] = `program_num'
		...
		matrix mediation_table_coef_se[`row',1] = r[1,1]
		matrix mediation_table_coef_se[`row',2] = r[2,1]
		matrix mediation_table_coef_se[`row',3] = r[1,2]
		matrix mediation_table_coef_se[`row',4] = r[2,2]
		matrix mediation_table_p[`row',1] = r[4,1]
		matrix mediation_table_p[`row',2] = r[4,2]
...
matrix mediation[`row',6] = abc_sb_ci[26,1]
matrix mediation[`row',7] = abc_sb_ci[975,1]
restore
...
cd "$mediation_out"
graph export "mediation_cognitive2.pdf", replace
...
cd "$mediation_out"
graph export "mediation_cognitive_selected.pdf", replace
...
local nrow = rowsof(interaction_p)
local ncol = colsof(interaction_p)

qui matrix interaction_stars = J(`nrow', `ncol', 0) // stars matrix

qui matrix colnames interaction_stars = `interaction_colnames'
qui matrix rownames interaction_stars = `interaction_rownames'

forvalues k = 1/`nrow' {
	forvalues l = 1/`ncol' {
		qui matrix interaction_stars[`k',`l'] = (interaction_p[`k',`l'] < 0.1) ///
			+ (interaction_p[`k',`l'] < 0.05) ///
			+ (interaction_p[`k',`l'] < 0.01)
	}
}

cd "$mediation_out"
frmttable using interaction2, statmat(interaction_coef_se) substat(1) sdec(3) fragment tex replace nocenter ///
	annotate(interaction_stars) asymbol(*,**,***)

*Table for Mediation Regression Estimates

local nrow = rowsof(mediation_table_p)
local ncol = colsof(mediation_table_p)

qui matrix mediation_table_stars = J(`nrow', `ncol', 0) // stars matrix

qui matrix colnames mediation_table_stars = `mediation_table_colnames'
qui matrix rownames mediation_table_stars = `mediation_table_rownames'

forvalues k = 1/`nrow' {
	forvalues l = 1/`ncol' {
		qui matrix mediation_table_stars[`k',`l'] = (mediation_table_p[`k',`l'] < 0.1) ///
			+ (mediation_table_p[`k',`l'] < 0.05) ///
			+ (mediation_table_p[`k',`l'] < 0.01)
	}
}

cd "$mediation_out"
frmttable using mediation_table, statmat(mediation_table_coef_se) substat(1) sdec(3) fragment tex replace nocenter ///
	annotate(mediation_table_stars) asymbol(*,**,***)
