...
local nrow : list sizeof global(later_home_types)
	...
	foreach t of global `p'_type {
		foreach c of local `p'_cogs {
			...
			foreach r of global later_home_types {
				...
				bootstrap r(ind_eff) r(dir_eff) r(tot_eff), reps(100): sgmediation `c'36 if !missing(D), mv(norm_home_`r'36) iv(R) cv($covariates)
...
foreach p of global programs_merge {
	...
	foreach c of local `p'_cogs {
		...
		rename row_3 row
		include "${code_path}/function/home_agg_later"
