...
foreach t of global ehs_type {
	cd "$subpop_working"
	use "ehs-control-subpop.dta", clear
	...
	* Rearranging matching IHDP
	...
	cd "$subpop_working"
	save ehs`t'-home-subpop-merge, replace
	...
	cd "$subpop_working"
	save ehs`t'-labor-subpop-merge, replace
}
...
foreach t of global ihdp_type {
	cd "$subpop_working"
	use "ihdp-control-subpop.dta", clear
	...
	cd "$subpop_working"
	save ihdp`t'-home-subpop-merge, replace
	...
	cd "$subpop_working"
	save ihdp`t'-labor-subpop-merge, replace
}
...
cd "$subpop_working"
use "abc-control-subpop.dta", clear
...
cd "$subpop_working"
save abc-home-subpop-merge, replace
...
cd "$subpop_working"
save abc-labor-subpop-merge, replace
...
foreach t of global care_type {
	cd "$subpop_working"
	use "care-control-subpop.dta", clear

	cd "$data_home"
	merge 1:1 id using care`t'-home-agg-participation, nogen nolabel keep(match)
	rename norm_home_*18 norm_home_*1y
	rename norm_home_*42 norm_home_*3y
	...
	* Rearranging matching IHDP
	...
	cd "$subpop_working"
	save care`t'-home-subpop-merge, replace
	...
	cd "$subpop_working"
	save care`t'-labor-subpop-merge, replace
}
