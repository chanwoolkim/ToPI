...
cd "$subpop_working"
	...
	* Create an empty matrix that stores ages, coefficients, p-values, lower CIs, and upper CIs.
	qui matrix `p'R`t'_1 = J(45, 3, .) // for randomisation variable
	qui matrix `p'R`t'_3 = J(55, 3, .) // for randomisation variable
		...
		* Loop over rows to fill in values into the empty matrix.
		forvalues r = 1/45 {
			...
			capture confirm variable home1_`r'
				...
				if `num' > 10 {
					* Run regression
					qui regress home1_`r' R $covariates if ``t'_cond'
					
					* r(table) stores values from regression (ex. coeff, var, CI).
					qui matrix list r(table)
					qui matrix r = r(table)
					
					qui matrix `p'R`t'_1[`r',2] = r[1,1]
					qui matrix `p'R`t'_1[`r',3] = r[4,1]
					}
				}
			}
		...
		* Loop over rows to fill in values into the empty matrix.
		forvalues r = 1/55 {
			...
			capture confirm variable home3_`r'
				...
				if `num' > 10 {		
					* Run regression
					qui regress home3_`r' R $covariates if ``t'_cond'
					
					* r(table) stores values from regression (ex. coeff, var, CI).
					qui matrix list r(table)
					qui matrix r = r(table)
					
					qui matrix `p'R`t'_3[`r',2] = r[1,1]
					qui matrix `p'R`t'_3[`r',3] = r[4,1]
				}
			}
		}
...
foreach t of local subpop_types {
	...
	use ehscenter-`t'-subpop-item-1, clear

	foreach p of global programs {
		merge 1:1 row_1 using `p'-`t'-subpop-item-1, nogen nolabel
	}

	rename row_1 row
	save item-`t'-subpop-1, replace

	use ehscenter-`t'-subpop-item-3, clear

	foreach p of global programs {
		merge 1:1 row_3 using `p'-`t'-subpop-item-3, nogen nolabel
	}

	rename row_3 row
	save item-`t'-subpop-3, replace
}
...
foreach age of numlist 1 3 {
	foreach t of local subpop_types {
		...
		graph dot ehscenterR`t'insig ehscenterR`t'0_1 ehscenterR`t'0_05 ///
				  ...
				  ihdpR`t'insig ihdpR`t'0_1 ihdpR`t'0_05 ///
