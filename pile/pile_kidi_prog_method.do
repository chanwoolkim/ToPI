...
local row = 1
	...
	* INTENSITY
	di `"INTENSITY: `prog`p'' ``prog`p''_`out`p''' "'
		...
		/*
svmat ITT, names(col)
keep numITT coeffITT lowerITT upperITT pvalITT
keep if numITT != .
...
label values numITT Outcomes
	twoway (scatter numITT coeffITT,ylabel(,val angle(360)) ) (rcap lowerITT upperITT numITT, horizontal) ///
	if _n==1|_n==2 ///
...
preserve
	...
	* Prepare Graph *
	...
	cd "$out"
	graph export "pile_kidi`num'_`M'.pdf", replace
	cd "$git_out"
	graph export "pile_kidi`num'_`M'.pdf", replace
restore
local num=`num'+1
...
cd "$out"
	graph export "pile_kidi_all_methods.pdf", replace
cd "$git_out"
	graph export "pile_kidi_all_methods.pdf", replace
