...
foreach age of numlist 3 {
	...
	cd "$pile_out"
	graph export "outcome_pile_R_subpop_`age'.pdf", replace
	
	cd "$pile_git_out"
	graph export "outcome_pile_R_subpop_`age'.png", replace

	include "${code_path}/function/outcome_graph_sep"
	
	cd "$pile_out"
	graph export "outcome_pile_R_subpop_`age'_sep.pdf", replace
	
	cd "$pile_git_out"
	graph export "outcome_pile_R_subpop_`age'_sep.png", replace
}
